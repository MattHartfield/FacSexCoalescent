			...
			while(yesrec != 1){
				...
				if(nbreaks == 1){
					...
					if( (isbpend == 1) && (isyetbp == 0) ){
						...
						for(j = 0; j < NMax; j++){
							if( *((*(GType + j)) + 0) == rands){
								if((*((*(GType + j)) + nbreaks) != (-1)) && (*((*(breaks + 1)) + nbreaks) == 0) && (isallI((*(GType + j)), nbreaks, 0, 1) != 1) ){
									yesrec = 1;
								}
								break;
							}
