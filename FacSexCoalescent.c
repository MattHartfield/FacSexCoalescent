...
char * treemaker(double **TFin, double thetain, double mind, double maxd, double **MTab, unsigned int MTRows, unsigned int Itot, unsigned int *nmutin, const gsl_rng *r);
void MTabRA(double **MTab, unsigned int *MTRows, unsigned int Itot);
...
void coalesce(unsigned int **indvs, unsigned int **GType, double **CTms ,unsigned int **TAnc, double Ttot, unsigned int *Nwith, unsigned int *Nbet, unsigned int Itot, unsigned int deme, unsigned int *rsex, unsigned int *nsex, unsigned int ex, unsigned int drec, unsigned int e2, unsigned int **breaks, unsigned int nsites, unsigned int *lrec, unsigned int nbreaks, const gsl_rng *r){
	...
	switch(ex)
	{
		...
		default:	/* If none of these cases chosen, exit with error message */
			fprintf(stderr,"Error: Non-standard coalescent case selected.\n");
			exit(1);
            break;
	}
...
void sexsamp(unsigned int **indvs, unsigned int *rsex, unsigned int *nsex, unsigned int *Nwith, unsigned int Ntot, const gsl_rng *r){
	...
	for(x = 0; x < d; x++){
		if(*(Nwith + x) != 0){		/* Avoiding errors if no WH in deme */
	...
	for(i = 0; i < lct2; i++){
    	...
    	if(i == 0){
    		...
    		*(Cheight + 0) = birthtime;
			...
			if(rmut1 != 0){
				if((*nmutin + rmut1) > MTRows){
					MTabRA(MTab,&MTRows,Itot);
				}
				for(a = 0; a < rmut1; a++){
					/* Indicating location of mutants */
					*((*(MTab + (*nmutin+a))) + 0) = gsl_ran_flat(r, mind, maxd);
					*((*(MTab + (*nmutin+a))) + (parent1+1)) = 1;
				}
				*nmutin += rmut1;
			}
			
			if(rmut2 != 0){
				if((*nmutin + rmut2) > MTRows){
					MTabRA(MTab,&MTRows,Itot);
				}
				for(a = 0; a < rmut2; a++){
					/* Indicating location of mutants */
					*((*(MTab + (*nmutin+a))) + 0) = gsl_ran_flat(r, mind, maxd);
					*((*(MTab + (*nmutin+a))) + (child1+1)) = 1;
				}
				*nmutin += rmut2;
			}
...
/* Re-allocating space to MTab table if too big */
void MTabRA(double **MTab, unsigned int *MTRows, unsigned int Itot){

	unsigned int j;
	
	*MTRows += (theta*sumrepsq(Itot));
	
	MTab = (double**)realloc(MTab,(*MTRows)*sizeof(double *));
	
	for (j = ((*MTRows)-(theta*sumrepsq(Itot))); j < *MTRows; j++){
		MTab[j] = NULL;
	}
	
	for (j = 0; j < *MTRows; j++){
		MTab[j] = (double*)realloc(MTab[j], (Itot+1)*sizeof(int));
	}
			...
			*/
			...
			if(NextT > (tls + tts)){ 	/* If next event happens after a switch, change rates of sex */
				...
				*/
				...
				/*
				for(x = 0; x < d; x++){
					printf("WCH %d: BCH %d: NWITH %d: NBET %d",*(WCH+x),*(BCH+x),*(Nwith+x),*(Nbet+x));
				}
				printf("\n");
				*/
				/*
			...
			for(j = 0; j < Itot; j++){
				if((*((*(CTms + j)) + x)) != 0){
					...
					*((*(TFin + count)) + 2) = *((*(TAnc + j)) + x);
					printf("%lf %lf %lf ",*((*(TFin + count)) + 0),*((*(TFin + count)) + 1),*((*(TFin + count)) + 2));
					printf("\n");					
					count++;
				}
			}
